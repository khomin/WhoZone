...
public:
    ...
    void onDetection(std::function<void(Detection& detection)> v);
    void onFrameSaved(std::function<void(std::string path)> v);
...
private:
    ...
    std::function<void(std::string path)> _onFrameSaved;
    std::thread _thread;
